class CatsBreedListModel {

  List<CatBreedModel> _catBreedsList = [];
  List<CatBreedModel> get catBreedsList => _catBreedsList;


  CatsBreedListModel({required catBreedsList}){
    _catBreedsList = catBreedsList;
  }

  CatsBreedListModel.fromJson(List json) {
    if (json != null) {
      _catBreedsList = <CatBreedModel>[];
      json.forEach((v) {
        _catBreedsList!.add(new CatBreedModel.fromJson(v));
      });
    }
  }
  ...
  List toJson() {
    List data = [];
    if (this._catBreedsList != null) {
      data = this._catBreedsList!.map((v) => v.toJson()).toList();
    }
    return data;
  }
}
